package org.sky.sys.utils.memached;
...
public class MemcachedUtils {
	...
	public static boolean add(String key, Object value) {
		try {
			if (get(key) != null) {
				MemcachedLogUtils
						.writeLog("Memcached add方法报错，key值："
								+ key
								+ "\r\n"
								+ exceptionWrite(new Exception(
										"Memcached内存缓存中已经存在该键值对")));
				return false;
			} else {
	...
	private static class MemcachedLogUtils {
		...
		private final static String PID = ManagementFactory.getRuntimeMXBean()
				.getName();// 通过找到对应的JVM进程获取PID
			...
			try {
				...
				if (osName.contains("Windows")) {
					...
					fileWriter = new FileWriter(
							"/usr/local/logs/memcached.log", true);
				}
	...
	public static void main(String args[]){
		...
		MemcachedUtils.add("123", user);
		System.out.print(MemcachedUtils.get("123").toString());
		**/
	}
}